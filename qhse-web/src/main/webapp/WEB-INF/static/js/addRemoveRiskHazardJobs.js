...
function autoresize(arrayEl){
	for(var i = 0; i < arrayEl.length; i++){
		var textarea = arrayEl[i];
		textarea.style.height= "54px";
		textarea.addEventListener('keydown', autosizeTxt);
	}
}
function autosizeTxt(){
    ...
    setTimeout(function(){
      ...
      el.style.cssText = 'height:' + (el.scrollHeight + 10) + 'px;overflow:hidden';
    },0);
	}
var arrayEl = new Array();
var select2Array = new Array();
function addRow(targetRow) {
	...
	for (var i = 0; i < rowTemplate.cells.length; i++) {
		...
		if (rowTemplate.cells[i].childNodes.length > 0) {
			...
			if(cell.getElementsByTagName('textarea').length > 0) {
				...
				arrayEl.push(cell.getElementsByTagName('textarea')[0]);
			}
			if(cell.getElementsByTagName('select').length > 0) {
				...
				if(jQuery(cell.getElementsByTagName('select')[0]).attr("multiple")){
					jQuery(cell.getElementsByTagName('select')[0]).removeClass("list-group");
					jQuery(cell.getElementsByTagName('select')[0]).removeClass("checked-list-box");
					jQuery(cell.getElementsByTagName('select')[0]).removeClass("select2-offscreen");
					jQuery(cell.getElementsByTagName('div')[0]).remove();
					jQuery(cell.getElementsByTagName('select')[0]).hide();
					select2Array.push(cell.getElementsByTagName('select')[0]);
				}
			}
			...
			if (rowTemplate.cells[i].firstChild.value == 'Del') {				
				cell.innerHTML = "<input type='button' h-id='' t-size=''  class='delete newDel btn btn-primary' id='"+rowIndex+"' onclick='deleteRow(getParent(\"tr\",this));' value='Del'/>";
			}
		}
	}setTimeout(function (){autoresize(arrayEl)}, 2000)
	...
	setTimeout(function (){
		jQuery("select[multiple]").each(function (){
			...
			jQuery(this).select2({width: '180px'});
		});
	}, 50);
