var App = function () {
  ...
  var dashboard = function(){
    ...
    skycons.add(jQuery("#sun-icon")[0], Skycons.PARTLY_CLOUDY_DAY);
    skycons.play();

  
    /*Sparklines*/
    jQuery(".spk1").sparkline([2,4,3,6,7,5,8,9,4,2,6,8,8,9,10], { type: 'bar', width: '80px', barColor: '#4A8CF7'});
    jQuery(".spk2").sparkline([4,6,7,7,4,3,2,1,4,4 ,5,6,5], { type: 'discrete', width: '80', lineColor: '#4A8CF7',thresholdValue: 4,thresholdColor: '#ff0000'});
    
    jQuery(".spk3").sparkline([5,6,7,9,9,5,3,2,2,4,6,7], {
    type: 'line',
    lineColor: '#258FEC',
    fillColor: '#4A8CF7',
    spotColor: false,
    ...
    if (jQuery('#site_statistics').size() != 0) {
      ...
      var plot_statistics2 = jQuery.plot(jQuery("#site_statistics2"), [{
        data: pageviews,
        label: "Unique Visits"
      }, {
        data: visitors,
      ...
      var plot_statistics = jQuery.plot(jQuery("#com_stats"), [{
      ...
      var plot_statistics2 = jQuery.plot(jQuery("#com2_stats"), [{
        data: data3,
        label: "Unique Visits"
      }
      ], {
      ...
      jQuery("#site_statistics2").bind("plothover", function (event, pos, item) {
    ...
    /*Jquery Easy Pie Chart*/
      jQuery('.epie-chart').easyPieChart({
        ...
        onStep: function(val){//Update current value while animation
          jQuery("span", this.jQueryel).html(parseInt(val) + "%");
        }
        });

    /*Chat Comments*/
    jQuery(".chat-in form").submit(function(e){

      var d = new Date();
      var time = d.getHours() + ":" + d.getMinutes();
      var inp = jQuery("input[type='text']", this);
      var msg = inp.val();
      var img = 'images/avatar1_50.jpg';
        
      inp.val("");
      
      var tpl = jQuery('<div class="chat-conv" style="display:none;">' +
      ...
      jQuery(this).parents(".chat-wi").find(".chat-content").css("transition", "all 1s ease");
      jQuery(this).parents(".chat-wi").find(".chat-content").append(tpl);
      
      tpl.slideDown();
      //jQuery(".nscroller").nanoScroller({ scroll: 'bottom' });
      //jQuery(".nscroller").nanoScroller();
      //jQuery(".nscroller").animate({scrollBottom:0}, '500', 'swing');
      e.preventDefault();
    });
  };
  /*END OF DASHBOARD*/
  ...
  var nestable = function(){
    ...
    function update_out(selector, sel2){
      var out = jQuery(selector).nestable('serialize');
      jQuery(sel2).html(window.JSON.stringify(out));
    }
    
    update_out('#list1',"#out1");
    update_out('#list2',"#out2");
    
    jQuery('#list1').on('change', function() {
      update_out('#list1',"#out1");
    });
    
    jQuery('#list2').on('change', function() {
      update_out('#list2',"#out2");
    });
  ...
  var wizard = function(){
    //Fuel UX
    jQuery('.wizard-ux').wizard();

    jQuery('.wizard-ux').on('changed',function(){
      //delete jQuery.fn.slider;
      jQuery('.bslider').slider();
    });
    
    jQuery(".wizard-next").click(function(e){
      var id = jQuery(this).data("wizard");
      jQuery(id).wizard('next');
      e.preventDefault();
    });
    
    jQuery(".wizard-previous").click(function(e){
      var id = jQuery(this).data("wizard");
      jQuery(id).wizard('previous');
      e.preventDefault();
    });
  };//End of wizard
  ...
  /*Form Masks*/
  var masks = function(){
    jQuery("[data-mask='date']").mask("99/99/9999");
    jQuery("[data-mask='phone']").mask("(999) 999-9999");
    jQuery("[data-mask='phone-ext']").mask("(999) 999-9999? x99999");
    jQuery("[data-mask='phone-int']").mask("+33 999 999 999");
    jQuery("[data-mask='taxid']").mask("99-9999999");
    jQuery("[data-mask='ssn']").mask("999-99-9999");
    jQuery("[data-mask='product-key']").mask("a*-999-a999");
    jQuery("[data-mask='percent']").mask("99%");
    jQuery("[data-mask='currency']").mask("jQuery999,999,999.99");
  };//End of masks
  ...
  var textEditor = function(){
    ...
    jQuery(".inline-editable").each(function(){
      CKEDITOR.inline(jQuery(this)[0]);
    });
  };//End of textEditor
  ...
  var charts = function(){
    ...
    if (jQuery('#site_statistics').size() != 0) {
      ...
      var plot_statistics = jQuery.plot(jQuery("#chart3"), [{
        ...
        legend:{
          show: true,
           ...
           container: jQuery("#chart3-legend")
        },
        grid: {
          show:false,
          margin: 0,
      ...
      var chart4 = jQuery.plot(jQuery("#chart4"), [{
        data:  getRandomData() ,
        label: "Sales"
      }
      ], {
      ...
      jQuery("#chart3").bind("plothover", function (event, pos, item) {
  ...
  var speech = function(options){
    ...
    if(('webkitSpeechRecognition' in window)){
      ...
      if(options=="start"){
        ...
        rec.onresult = function(event){
          for (var i = event.resultIndex; i < event.results.length; ++i) {
            if (event.results[i].isFinal) {
              ...
              command = command.replace(/^\s+|\s+jQuery/g,'');//Trim spaces
              console.log(command);
              if(config.onResult){
                config.onResult(command);
              }   
              
              jQuery.each(speech_commands,function(i, v){
        ...
        rec.onaudiostart = function(){ jQuery(".speech-button i").addClass("blur"); }
        rec.onend = function(){
          jQuery(".speech-button i").removeClass("blur");
    ...
    if(command){
      ...
      function updateHeight(){
        if(!jQuery("#cl-wrapper").hasClass("fixed-menu")){
          ...
          var navH = jQuery("#qhse_head_nav").height();
          ...
          // var height = (jQuery("#pcont").height() < jQuery(window).height())?jQuery(window).height():jQuery(document).height();
    ...
    init: function (options) {
      ...
      jQuery(".cl-vnavigation").delegate(".parent > a","click",function(e){
        ...
        ul.slideToggle(300, 'swing', function () {
          ...
          //var menuH = jQuery("#cl-wrapper .menu-space .content").height();
          // var height = (jQuery(document).height() < jQuery(window).height())?jQuery(window).height():menuH;
          //updateHeight();
         jQuery("#cl-wrapper .nscroller").nanoScroller({ preventPageScrolling: true });
        });
        e.preventDefault();
      ...
      /*DateTime Picker*/
      if(config.dateTime){
        jQuery(".datetime").datetimepicker({autoclose: true});
      }
...
jQuery(function(){
  //jQuery("body").animate({opacity:1,'margin-left':0},500);
