	...
	<div id="block" class="">
		<div>
		    ...
		    <div class="col-md-12 col-sm-12">
		    	<div align="right">
		     <form action="<c:url value="/change/templateView.htm" />" method="get" onSubmit="if(!jQuery('#gotoId')) return false;">
			    <fmt:message key="change.templateQueryForm.goTo"/>
			    ...
			    <button type="button" class="g-btn g-btn--primary" id="queryTableToggleLink" onclick="toggleQueryTable();">&nbsp;<fmt:message key="search.hideSearch"/></button>
					<c:if test="${addButtonEnabled == true }">
						<button type="button" class="g-btn g-btn--primary" onclick="location.href='templateEdit.htm'"><i class="fa fa-edit" style="color:white"></i>&nbsp;<fmt:message key="newChangeTemplate"/></button>
					</c:if>
...
<div class="content" >
			...
			<form id="queryForm"  class="form-horizontal group-border-dashed" style="border-radius: 0px;"  action="<c:url value="/change/templateQuery.htmf" />" onSubmit="return search(this, 'resultsDiv'); ">
