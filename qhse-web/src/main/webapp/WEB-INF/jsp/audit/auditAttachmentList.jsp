...
<c:forEach items="${attachmentList}" var="item">
	<tr>
		<td><a href="<c:url value="auditAttachmentEdit.htm"><c:param name="itemId" value="${item.id}" /><c:param name="showId" value="${parentId}" /></c:url>"><c:out value="${item.name}" /></a><br /> </td>
...
<tfoot>
	<tr>
		<td colspan="5">
			...
			<a href="<c:url value="auditAttachmentEdit.htm"><c:param name="showId" value="${parentId}"/></c:url>"><fmt:message key="addAttachment" /></a>
			</c:if>
		</td>
	</tr>
</tfoot>
