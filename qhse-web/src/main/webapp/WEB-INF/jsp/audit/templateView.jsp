...
<body>
<div class="header">
<h2><fmt:message key="templateView" /></h2>
</div>
<div class="content">
...
<tbody>
	...
	<tr>
		<td class="scannellGeneralLabel"><fmt:message key="scorable" />:</td>
	...
	<tr>
		<td class="scannellGeneralLabel nowrap"><fmt:message key="auditTemplate.notIncidentAssigned"/>:</td>
		<td><fmt:message key="${template.assignIncidentToAuditee}" /></td>
	</tr>
	<tr>
		<td class="scannellGeneralLabel nowrap"><fmt:message key="defaultFindingType"/>:</td>
		<td>
		<c:if test="${template.defaultFindingType == null}"><fmt:message key="choose" /></c:if>
		<c:if test="${template.defaultFindingType != null}"><fmt:message key="${template.defaultFindingType}" /></c:if>
		</td>
	</tr>
	<tr>
...
<c:forEach items="${groups}" var="groupQuestionEntry">
	...
	<tr>
		<th colspan="3" bgcolor="#E9F1E4">
			<c:if test="${group eq null}"><fmt:message key="auditTemplate.nullGroup" /></c:if>
			<a href="<c:url value="auditTemplateQuestionGroup.htm"><c:param name="id" value="${group.id}" /></c:url>"><c:out value="${group.name}" /></a>
		</th>
	</tr>
