			...
			<div class="col-md-12 col-sm-12">
				<div align="right">
					...
					<c:if test="${addButtonEnabled == true }">
						<button type="button" class="g-btn g-btn--primary" onclick="location.href='reportingGroupEdit.htm'">
							<i class="fa fa-edit" style="color: white"></i>
							<fmt:message key="groupReportingCreate" />
						</button>
					</c:if>
...
<div class="header">
<h2><fmt:message key="reportingGroupList" /></h2>
</div>
<div class="content">
  ...
  <tbody>
    <c:forEach items="${groups}" var="group" varStatus="s">
      ...
      <tr class="<c:out value="${style}" />">
        <td><a href="<c:url value="reportingGroupView.htm"><c:param name="id" value="${group.id}"/></c:url>" ><c:out value="${group.name}" /></a></td>
      </tr>
    </c:forEach>  
   </tbody>
  <tfoot>
