...
<body>
<div class="header">
<h2><fmt:message key="editAttachments" /></h2>
</div>
...
<tbody>
	<%-- <c:choose>
		<c:when test="${record_type == 'training'}">
			<c:set var="pageTitleForBackToLink" value="maintenance.trainingView.title"/>
			<c:set var="pageTitleForBackToLink" value="maintenance.trainingView.title"/>
		</c:when>
		<c:when test="${record_type == 'equipment'}">
			<c:set var="pageTitleForBackToLink" value="maintenance.equipmentView.title"/>
		</c:when>
	</c:choose> --%>
  <c:forEach items="${record.attachments}" var="item">
  <tr>
    <td>
      <a href="<c:url value="${attachment_link_url}"><c:param name="itemId" value="${item.id}" /><c:param name="showId" value="${command.training.id}" /></c:url>"><c:out value="${item.name}" /></a>
