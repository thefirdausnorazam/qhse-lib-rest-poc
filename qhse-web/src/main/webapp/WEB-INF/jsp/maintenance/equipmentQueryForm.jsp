...
<script type="text/javascript">
	...
	function resetForm() {
		jQuery('select').not('#siteLocation').not('#pageSize').not('#sortName')
				.not('.recordsPerPage > select').select2("val", "");
	...
	function restoreSearchCriteria() {
		jQuery("#categoryId").val(
				'${sessionScope["equipmentSearch.categoryId"]}');
		if('${sessionScope["equipmentSearch.active"]}' == '') {
			jQuery("#active").val('true');
		} else {
			jQuery("#active").val('${sessionScope["equipmentSearch.active"]}');
		}
		
		jQuery("#dueDateFrom").val(
				'${sessionScope["equipmentSearch.dueDateFrom"]}');
		jQuery("#dueDateTo")
				.val('${sessionScope["equipmentSearch.dueDateTo"]}');
		jQuery("#lastDateFrom").val(
				'${sessionScope["equipmentSearch.lastDateFrom"]}');
		jQuery("#lastDateTo").val(
				'${sessionScope["equipmentSearch.lastDateTo"]}');
		jQuery("#userTrainee").val(
				'${sessionScope["equipmentSearch.userTrainee"]}');
		jQuery("#thirdPartyTrainee").val(
				'${sessionScope["equipmentSearch.thirdPartyTrainee"]}');
		jQuery("#company").val('${sessionScope["equipmentSearch.company"]}');
		jQuery("#assetId").val('${sessionScope["equipmentSearch.assetId"]}');
		if('${sessionScope["equipmentSearch.sortName"]}' != '' && '${sessionScope["equipmentSearch.sortName"]}' != 'null'){
			jQuery("#sortName").val('${sessionScope["equipmentSearch.sortName"]}');
		}
		jQuery("#serialNo").val('${sessionScope["equipmentSearch.serialNo"]}');
		jQuery("#location").val('${sessionScope["equipmentSearch.location"]}');
		jQuery("#carriedOutBy").val(
				'${sessionScope["equipmentSearch.carriedOutBy"]}');
		jQuery("#description").val(
				'${sessionScope["equipmentSearch.description"]}');
		jQuery("#categoryId").val(
				'${sessionScope["equipmentSearch.categoryId"]}');
		/*   if(${sessionScope["equipmentSearch.active"]}){
			  $jq("#active").val('${sessionScope["equipmentSearch.active"]}');
			  }else{
				  $jq("#active").val('No');
			  } */
		jQuery("#dueDateFrom").val(
...
<body>
	<div class="col-md-12">
		<div id="block" class="">
			<div>
				...
				<form action="<c:url value="/maintenance/equipmentRecordView.htm" />" method="get"
						onsubmit="if(!jQuery('#gotoId')) return false;">
						<fmt:message key="maintenance.equipmentQueryForm.goTo" />
						...
						<c:if test="${addButtonEnabled == true }">
							<button type="button" class="g-btn g-btn--primary" onclick="location.href='equipmentEdit.htm'">
								<i class="fa fa-edit" style="color: white"></i>
								<fmt:message key="equipmentCreate" />
							</button>
						</c:if>
	...
	<div class="content">
		<form id="queryForm" action="<c:url value="/maintenance/equipmentQuery.htmf" />"
								...
								<tbody>
									<tr class="form-group">
										<td class="searchLabel"><label for="categoryId">
												<fmt:message key="equipment.category" />
											</label></td>
									...
									<tr class="form-group">
										<td class="searchLabel"><label for="dueDateFrom">
												<fmt:message key="equipment.dueDate" />
									...
									<tr class="form-group">
										<td class="searchLabel"><label for="lastDateFrom">
												<fmt:message key="equipment.lastDate" />
												<fmt:message key="from" />
											</label></td>
									...
									<tr class="form-group">
										<td class="searchLabel"><label for="userTrainee">
												<fmt:message key="equipment.responsible" />
												(
									...
									<tr class="form-group">
										<td class="searchLabel"><label for="assetId">
												<fmt:message key="equipment.assetId" />
											</label></td>
										<td class="search"><input type="text" id="assetId" name="assetId" style="width: 50%" /></td>
										<td class="searchLabel"><label for="serialNo" class="search">
												<fmt:message key="equipment.serialNo" />
											</label></td>
										<td class="search"><input type="text" id="serialNo" name="serialNo" style="width: 50%" /></td>
									</tr>

									<tr class="form-group">
										<td class="searchLabel"><label for="location">
												<fmt:message key="equipment.location" />
											</label></td>
										<td class="search"><input type="text" id="location" name="location" style="width: 50%" /></td>
										<td class="searchLabel"><label for="carriedOutBy" class="search">
												<fmt:message key="equipment.carriedOutBy" />
											</label></td>
										<td class="search"><input type="text" id="carriedOutBy" name="carriedOutBy" style="width: 50%" /></td>
									</tr>
									...
									<tr class="form-group">
										...
										<td class="search"><select id="sortName" name="sortName" class="wide selectSize300">
												<option value="id"><fmt:message key="choose" /></option>
										...
										<td class="searchLabel nowrap"><span style="display: none;"><fmt:message key="maintenance.equipmentQueryForm.recordsPerPage" /></span></td>
										<td class="search"></td>
									</tr>
								</tbody>
