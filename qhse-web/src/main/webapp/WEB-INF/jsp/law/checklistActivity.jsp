...
<%@ taglib prefix="law" tagdir="/WEB-INF/tags/law" %>
    ...
    jQuery.ajaxSetup({
      ...
      scansol = {}; // namespace
      scansol.relevant='${sessionScope["checklist.relevant"]}';
      scansol.refine='${sessionScope["checklist.refine"]}';
      if(scansol.relevant == 'true'){
      ...
      function collapseChecklists() {
          ...
          //$jq("div.law").append('<div class="checklist-toggleall"><a target="_self" href="javascript:void();" ss-action="*" >Show Relevant</a> | <a target="_self" href="javascript:void();" ss-action="*">Show Not Relevant</a> | <a target="_self" href="#" ss-action="+">Expand All</a> | <a target="_self" href="#" ss-action="-">Hide All</a></div>');
           ...
           $jq("div.checklist-toggleall a").click(function() {
    	        ...
    	        if(expand == '+'){
    	          ...
    	          if(scansol.refine == "true"){
    	        	window.location.href=contextPath+'/law/checklists.htm?refine=true&viewAll=true&viewType=relevant&legRegister='+legRegister;
    	          }else{
    	         	window.location.href=contextPath+'/law/checklists.htm?refine=false&viewAll=true&viewType=relevant&legRegister='+legRegister;
    	          }
    	        }else{
    	        	if(scansol.refine == "true"){
    	        		window.location.href=contextPath+'/law/checklists.htm?refine=true&viewAll=true&viewType=notRelevant&legRegister='+legRegister;
    	        	}else{
    	        		window.location.href=contextPath+'/law/checklists.htm?refine=false&viewAll=true&viewType=notRelevant&legRegister='+legRegister;
    	        	}
    	        }
    	        return false;
          });
      ...
      var actions = {
        ...
        notRelevant:function(data) {
        	var params = data.profileChecklistId > 0 ? "showId=" + data.profileChecklistId :  "profile=" + data.profileId + "&checklistId=" + data.checklistId;
        	params=params+"&markRelevant="+(scansol.relevant=='true' ? 'false' : 'true');
        ...
        relevant:function(data) {
         ...
         expand:function(data) {
        	$('html').children('thead').each(function () {
	            $jq(this).closest(".checklist-toggle a").html("Hide Detail");
	            var checklistid = $jq(this).attr('checklistid');
	            var profileId = $jq(this).attr('profileId');
	            var url = contextPath+"/law/showChecklistDetail.htmf?checklistId="+checklistid+"&profileId="+profileId;
	            
	        	var checklistDiv = $jq('tbody[checklistid="' + checklistid + '"] div.showChecklistDetail');
	        	
		       checklistDiv.mask("Waiting...");
		       checklistDiv.load(url, function() {
		            checklistDiv.find(".profileChecklistToolbar").html(scansol.toolbarHtml);
      ...
      if(scansol.refine == "true"){
	       scansol.toolbarHtml = [
	         ...
	         '<a target="_self" ss-action="addTask" href="#">Add Task</a> | ',
	          ...
	          '<a target="_self" ss-action="notRelevant" href="#">'+scansol.toggleLabel+'</a> ',
	         '</div>' 
	      ].join("");
      }else{
	       scansol.toolbarHtml = [
      ...
      jQuery(".profileChecklistToolbar").html(scansol.toolbarHtml);
      ...
      scansol.checklistEditDialogClose = function() {
        ...
        relevanceDiv.load(checklistRelevanceDetailUrl, function() {
          relevanceDiv.unmask();
          relevanceDiv.find(".profileChecklistToolbar").html(scansol.toolbarHtml);
