...
<script type="text/javascript">
	jQuery(document).ready(function() {
		initSortTablesForClass('dataTableObjective');
	} );
	if('${searchByLegacyId}' == 'true'){
		if('${fn:length(result.results)}' == '1'){
			location.href = '${pageContext.request.contextPath}' + '/risk/objectiveView.htm?id=${result.results[0].id}';
		}
	}
</script>
...
<div class="panel" > 
				<c:choose>
					    <c:when test="${groupSite == true}">
					    	<c:forEach items="${sites}" var="site" varStatus="s"> 
					    		<table class="table table-bordered table-responsive dataTableObjective">
		      ...
		      <risk:objective objective="${objective}" style="${style}" showSiteColumn="${showSiteColumn}" showLegacyIds="${showLegacyId}"/>
								    </c:forEach>
								<tfoot>
								    <c:if test="${found}">
									    <tr>
