...
<h2><fmt:message key="editRiskAttachments" /></h2>
</div>
<div class="content">  
...
<tbody>
  ...
  <c:forEach items="${riskObjective.attachments}" var="item">
  
  <tr>
    <td>
      <a href="<c:url value="objectiveAttachmentEdit.htm"><c:param name="itemId" value="${item.id}" /><c:param name="showId" value="${riskObjective.id}" /></c:url>"><c:out value="${item.name}" /></a>
  ...
  <tr>
    <td colspan="7">
     <c:if test="${riskObjective.editable}">
        <a href="<c:url value="objectiveAttachmentEdit.htm"><c:param name="showId" value="${riskObjective.id}"/></c:url>">
