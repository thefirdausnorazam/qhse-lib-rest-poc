...
<head>
	<title>
		<c:choose>
			<c:when test="${command.id == 0}"><fmt:message key="doccontrol.documentReview.create" /></c:when>
			<c:otherwise><fmt:message key="doccontrol.documentReview.edit" /></c:otherwise>
		</c:choose>
	</title>
...
<%-- <div class="header ">
<h2><fmt:message key="doccontrol.documentReview.create" /></h2>
</div> --%>
<scannell:form>
	<div class="content">
		<div class="form-group" style="min-height:110px;">
			<label class="col-sm-3 control-label scannellGeneralLabel nowrapl rightAlign">
				<fmt:message key="reviewComment" />:
			</label>
			<div class="col-sm-6" >
				<scannell:textarea path="comment" cols="75" rows="3" />
			</div>
		</div>
