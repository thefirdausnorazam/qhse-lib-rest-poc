...
<body>
	<script type="text/javascript">
		jQuery(document).ready(function() {
			initSortTablesForClass('dataTableApprove');
		} );
	</script>
			
		<!--  My Documents to Approve -->
		<c:set var="found" value="${!empty approveDocuments.results}" />
		<div class="header">
			<h3>
				<fmt:message key="doccontrol.approveDocuments" />
			</h3>
		</div>
		<div class="content">
			<div class="table-responsive">
				<div class="panel">
				    <c:if test="${found}">
				         <div class="div-for-pagination"><scannell:paging result="${approveDocuments}" /></div>
				    </c:if>
					<table class="table table-bordered table-responsive dataTableApprove">
						<thead>							
								<tr>
						...
						<tbody>
							...
							<c:forEach items="${approveDocuments.results}" var="approveDoc" varStatus="s">
								<tr>
						...
						<tfoot> 
						    <c:if test="${found}">
							    <tr>
							      <td colspan="5"><scannell:paging result="${approveDocuments}" /></td>
							    </tr>
						    </c:if>
					    </tfoot>
					</table>
